...
vector<vector<int>> avgset(vector<int> &A)
{
    ...
    // size of 1st subset can be from 1 to (n-1)(since both subsets need to be non-empty). if for a given n1,
    //  s1 comes out to be decimal we can straight away say that we can't find sum s1 in A
